...
def get_result(args):
    ...
    w = args.post_config["SHAPES_DECODE"][0]
    h = args.post_config["SHAPES_DECODE"][1]

    while True:
        args.ret = args.instance.get_result()
        args.anno = args.post.main(args.ret['buf'].view(np.float32), np.array([[0, 0, w-1, h-1]], dtype=np.float32))
